...
func init() {
	...
	cbcDownloadCmd.Flags().StringVarP(&Version, "version", "v", "2.138.4.3", "The version for CloudBees Core, or use 2.138.4.3 by leaving empty")
	...
	deleteNamespaceCmd.Flags().StringVarP(&Name, "name", "n", "", "The name of the namespace")

	deleteCmd.AddCommand(deleteNamespaceCmd)
	deleteCmd.AddCommand(deleteLECRDSCmd)
	deleteCmd.AddCommand(deleteLEHelmCmd)
...
var deleteCmd = &cobra.Command{
	Use:   "delete",
	Short: "Will delete subresources",
	Long:  `Anything to do with deletes`,
	Args:  cobra.MinimumNArgs(1),
	Run:   func(cmd *cobra.Command, args []string) {},
}

var deleteLECRDSCmd = &cobra.Command{
	Use:   "letsencrypt-crds",
	Short: "Will delete letsencrypt-crds",
	Long:  `Anything to do with deletes`,
	Run: func(cmd *cobra.Command, args []string) {
		letsencrypt.CleanCRDS()
	},
}

var deleteLEHelmCmd = &cobra.Command{
	Use:   "letsencrypt-helm",
	Short: "Will delete letsencrypt-helm",
	Long:  `Anything to do with deletes`,
	Run: func(cmd *cobra.Command, args []string) {
		letsencrypt.PurgeHelm()
	},
}

var deleteNamespaceCmd = &cobra.Command{
	Use:   "namespace",
	Short: "Will delete a namespace",
	Long:  `Will delete a namespace specified with -n`,
	Run: func(cmd *cobra.Command, args []string) {
		if Name == "" || len(Name) < 2 {
			log.Fatal("Must have a name with at least two characters")
		}
...
var installLetsEncryptCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		err := validation.Errors{"email": validation.Validate(Email, validation.Required, is.Email)}.Filter()
		if err != nil {
